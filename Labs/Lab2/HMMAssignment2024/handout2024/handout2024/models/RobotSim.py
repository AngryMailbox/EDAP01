...
class RobotSim:
    def __init__(self, state, sm):
        """Initializes the robot simulator with the given state and state model."""
        self.__currentState = state
        print("starting in state {}".format(state))
        self.__sm = sm

    def move_once(self, tm) -> int:
        """Moves the robot according to the transition model tm and returns the new state."""
        newState = -1
        rand = random.random()
    ...
    def sense_in_current_state(self, om) -> int:
        """Returns the sensor reading in the current state. The sensor model is sent in as an argument."""
        sensorReading = None
