...
sys.path.append(
    "C:/Users/mans/Documents/projekt/AI/Lab2/HMMAssignment2024/handout2024/handout2024"
)
...
class Dashboard:
    ...
    def __init__(self, ROOM_HEIGHT, ROOM_WIDTH, UniformF):
        self.slider_h = widgets.IntSlider(
            min=ROOM_HEIGHT, max=10, step=1, description="Height", value=ROOM_HEIGHT
        )
        self.slider_w = widgets.IntSlider(
            min=ROOM_WIDTH, max=10, step=1, description="Width", value=ROOM_WIDTH
        )
        self.slider_h.observe(self.on_slider_change, names="value")
        self.slider_w.observe(self.on_slider_change, names="value")

        self.slider_f = widgets.IntSlider(
            min=0, max=1, step=1, description="Sensor: 0 / 1", value=UniformF
        )
        self.slider_f.observe(self.on_slider_change, names="value")

        self.out = widgets.Output(
            layout=widgets.Layout(height="720px", width="720px", border="solid")
        )
        ...
        self.animation = widgets.HBox(
            [self.btn_if, self.btn_os, self.btn_go, self.btn_sp]
        )
    ...
    def btn_ss_eventhandler(self, obj):
        ...
        try:
            ...
            for state in range(self.num_states):
                ...
                self.visualizationroom[
                    visrow, viscol
                ] = self.model.get_observation_model().get_o_reading_state(
                    self.observation_step, state
                )

                if (
                    self.observation_step == self.room.state_to_reading(state)
                    and h == 0
                    and self.observation_step != self.rows * self.cols
                ):
    ...
    def btn_if_eventhandler(self, obj):
        ...
        try:
            ...
            for state in range(self.num_states):
                ...
                visrow = r * 3 + self.visrow_iter[h]
                viscol = c * 3 + self.viscol_iter[h]
                self.visualizationroom[visrow, viscol] = (
                    self.model.get_current_f_vector()[state]
                )
    ...
    def update_grid_one_step(self, plotting):
        ...
        try:
            ...
            if sensed:
                self.visualizationroom[sensedR * 3 + 1, sensedC * 3 + 1] = (
                    129 / 256 * 2
                )  # turquoise
            self.visualizationroom[guessedR * 3 + 1, guessedC * 3 + 1] = (
                127 / 256 * 2
            )  # red
            self.visualizationroom[trueR * 3 + 1, trueC * 3 + 1] = (
                256 / 256 * 2
            )  # black
            ...
            if sensed:
                print(
                    "true pose = <{}, {}, {}>, sensed position = <{}, {}>, guessed position = <{}, {}>".format(
                        trueR, trueC, trueH, sensedR, sensedC, guessedR, guessedC
                    )
                )
            else:
                print(
                    "true pose = <{}, {}, {}>, sensed nothing".format(
                        trueR, trueC, trueH
                    )
                )
            ...
            self.nbr_of_moves += 1
            print(
                "nbr of moves: {}, avg error: {}, nbr correct guesses: {}".format(
                    self.nbr_of_moves,
                    self.total_error / self.nbr_of_moves,
